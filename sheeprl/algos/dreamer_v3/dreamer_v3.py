...
def train(
    ...
    rec_loss, kl, state_loss, reward_loss, observation_loss, continue_loss, dyn_loss, repr_loss = reconstruction_loss(
        po,
        batch_obs,
        pr,
        data["rewards"],
    ...
    if aggregator and not aggregator.disabled:
        ...
        aggregator.update("User/DynLoss", dyn_loss.detach())
        aggregator.update("User/ReprLoss", repr_loss.detach())
    ...
    if aggregator and not aggregator.disabled:
        ...
        aggregator.update("User/LambdaValues", lambda_values.detach())
        aggregator.update("User/Advantages", advantage.detach())
        aggregator.update("User/Entropy", entropy.detach())

        aggregator.update("User/PredictedRewards", predicted_rewards.detach())
        aggregator.update("User/PredictedValues", predicted_values.detach())
...
def main(fabric: Fabric, cfg: Dict[str, Any]):
    ...
    for update in range(start_step, num_updates + 1):
        ...
        # print("step rewards: ", step_data["rewards"])
        test.append(step_data["rewards"])
        ...
        # Log metrics
        if cfg.metric.log_level > 0 and (policy_step - last_log >= 100 or update == num_updates):
            # if True:
            ...
            if not timer.disabled:
                ...
                if "Time/train_time" in timer_metrics:
                    fabric.log(
                        "Time/sps_train",
                        (train_step - last_train) / max(timer_metrics["Time/train_time"], 1),
                        policy_step,
                    )
